...
import { createShadowGenerator } from "./sceneset_shadowManager.js";
import { getUniqueId } from "./sceneset_scene.js";
...
export function createLight(type, savedData = null, scene) {
	...
	if (light) {
		if (savedData) {
			...
			// NEW: Restore visibility if saved
...
function setupLightSync(proxy, light, scene) {
	...
	proxy._lightObserver = scene.onBeforeRenderObservable.add(() => {
		...
		// NEW: Sync Enabled State (Visibility)
		// This ensures that if the proxy (gizmo) is hidden (via UI "Visible" toggle), the light is turned off
